  ...
  for (i in unique(df$puid)){
    ...
    for (x in 1:rep){
      ...
      if (dim(df.tot.cost)[1]>0){
        agg.benefits <- aggregate(x = df.tot.cost[c("prob.property","bid.price","cons.benefit")], by = df.tot.cost[c("puid")], FUN = sum)
        agg.benefits$npv <- mean(df.tot.cost$npv)
        agg.benefits$admin.cost <- mean(df.tot.cost$admin.cost)
        agg.benefits <- agg.benefits[c("puid", "npv", "admin.cost", "prob.property", "bid.price", "cons.benefit")]
    ...
    if (dim(result)[1]>0){
      agg.result <- aggregate(x = result[c("npv","admin.cost", "prob.property","bid.price","cons.benefit")], by = result[c("puid")], FUN = mean)
...
make.maps <- function(outfoldermaps, scen){
  ...
  solution <- data.frame(df_new$puid, binary.sol)
  selected_pus_scen1 <- solution$df_new.puid[solution$binary.sol==1]
  ...
  pu.and.budget <- data.frame(df_new$puid, budget.sol)
  colnames(pu.and.budget) <- c("CADID", "budget.sol")
  ...
  ##Join budget values to shp file
  pu.and.consben <- data.frame(df_new$puid, consben.sol)
